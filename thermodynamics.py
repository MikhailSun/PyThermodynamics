...
#вычисление стехиометрической постоянной, работает только для CH4, если нужно другое топливо - нужно изменить значения x и y в реализации функции
def Stoichiometric_const(mass_comp_air, mass_comp_fuel):
    ...
    #x=1;y=4
    # x=11.973; y=23.316
    if mass_comp_fuel[6]>0.999 and mass_comp_fuel[7]<0.001: #если топливо - жидкий керосин
        # CxHy + (y/4+x) O2 = y/2H2O + xCO2
        x = 11.973; y = 23.316
        fuel_id=6
        MolW_fuel=MolW_JetA
    elif mass_comp_fuel[7]>0.999 and mass_comp_fuel[6]<0.001: #если топливо - метан
        x=1; y=4
        fuel_id=7
        MolW_fuel =MolW_CH4
    ...
    G_fuel_brnt=1*fuel_id
    Nu_fuel_brnt=G_fuel_brnt/MolW_fuel #MolW_CH4
...
def Low_heat_of_combustion(mass_comp_air,mass_comp_fuel,eff=1,L0=np.nan):
    L0=Stoichiometric_const(mass_comp_air, mass_comp_fuel) if np.isnan(L0) else L0
    G_fuel=1
    G_fuel_brnt=G_fuel*eff
    G_air=L0*G_fuel
    mass_comp_comb_prod=Combustion_properties(mass_comp_air, mass_comp_fuel, G_fuel, G_air, eff, L0)[1]
    return ((H(298.15, mass_comp_fuel) * G_fuel + H(298.15, mass_comp_air) * G_air) - H(298.15, mass_comp_comb_prod) * (G_fuel_brnt+G_air) - H(298.15, mass_comp_fuel) * (G_fuel-G_fuel_brnt)) / G_fuel

#функция для вычисления свойств продуктов сгорания на выходе из КС по параметрам известным на входе в КС.
# если не задан стехиометрический коэффициент L0, то все свойства вычисляются исходя из хим формулы горения
...
def Combustion_properties(mass_comp_air, mass_comp_fuel, G_fuel, G_air, eff_brn, L0=np.nan):
    ...
    if mass_comp_fuel[6]>0.999 and mass_comp_fuel[7]<0.001: #если топливо - жидкий керосин
        ...
        fuel_id=7
    #второй вариант. В книге Дубовкин Маланичева "Физ хим и эксплуатационные свойства реактивных топлив", Москва, 1985, стр.92, табл.3.1 указано, что теплотворная способность ТС-1 43,21-43,37МДж/кг, там же стехиометрическая постоянная примерно 14,73 (стр.114). Отсюда искусственно подбираем условный состав топлива так, чтобы Hu и L были соответствующими C12H23.325
    G_fuel_brnt=G_fuel*fuel_id*eff_brn
    G_fuel_not_brnt=G_fuel*fuel_id*(1-eff_brn)#допущение: несгоревшее топливо осталось в продуктах сгорания в виде газообразного керосина
    if np.isnan(L0): #считаем состав продуктов сгорания по химической формуле CH4 + 2O2 = CO2 + 2H2O /горения керосина
        Nu_fuel_brnt = G_fuel_brnt / MolW_JetA #MolW_CH4
        Nu_O2 = (x + y / 4) * Nu_fuel_brnt  # в литературе 17,875 называется обычно стехиометрическим коэффициентом, который говорит сколько нужно атомов кислорода для полного сгорания топлива
        G_O2_from_brnt_air = Nu_O2 * MolW_O2
        G_N2_from_brnt_air = G_O2_from_brnt_air * mass_comp_air[0] / mass_comp_air[1]
        G_CO2_from_brnt_air = G_O2_from_brnt_air * mass_comp_air[3] / mass_comp_air[1]
    ...
    mass_comp_gas[5]=G_fuel_outlet/G_comb_prod
    mass_comp_gas[6]=0 #все топливо либо сгорело, либо испарилось
    mass_comp_gas[7]=0#G_fuel_outlet/G_comb_prod #это для газа CH4
...
# mass_comp_fuel= np.array([0.0000e+00, 0.0000e+00, 0.0000e+00, 0.0000e+00, 0.0000e+00, 0.0000e+00, 0.0000e+00,1.0])
# L0=Stoichiometric_const(dry_air_test,mass_comp_fuel)
# Hu=Low_heat_of_combustion(dry_air_test,mass_comp_fuel,eff=0.99,L0=17)
#
#
# x=Combustion_properties(dry_air_test,mass_comp_fuel,1,20,1)
# y=Combustion_properties(dry_air_test,mass_comp_fuel,1,20,1,L0=17.23858253663055)
# pass
