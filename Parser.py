...
#TODO! сделать возможность задавать константную функцию типа " f=10", где f - воспринимается как функция без аргументов и к ней можно обращаться как к обычной функции типа f()
#TODO! также ко всем функциям должно быть можно обращаться из штуки, которая формирует список с результатами
import ast
import numpy as np
import logging
import re
...
class Parser_formula():
    ...
    def prepare_formula(self, formula_string): #этот метод вызывается в разделе формул, которые определяется пользователь вида F(x,y,...)=x+y+... В методе разбирается только часть левее знака равно, и вызывается отдельный метод для разбора правой части уравнения
        ...
        _rez=re.search(r'([a-zA-Z0-9_]+)\({1}(.*)\){1}',LHS) #эта штука не ищет функции, у которых в названии нет скобок, например f(x) - находит, а просто f - нет
