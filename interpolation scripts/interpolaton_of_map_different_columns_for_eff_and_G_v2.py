...
name_of_file="2020 08 аэродинамические характеристики ТК ТВ7-117 по результатаам CFD\TK CFD 2020 08.csv"#!!!указать файл
...
# globalGmax=max(reader.loc[:,'G'])
# globalPRmax=max(reader.loc[:,'PR'])
...
#2)разбиваем исходные данные по столбцам и преобразуем их во float
# G=pd.to_numeric(reader.G)
# PR=pd.to_numeric(reader.PR)
# Eff=pd.to_numeric(reader.Eff)
# n=pd.to_numeric(reader.n)
# if type_of_map=="turbine":
#     Gn = pd.to_numeric(reader.Gn)
#     if 'L' in reader:
#         L=pd.to_numeric(reader.L) #для турбины
#     if 'A' in reader:
#         A=pd.to_numeric(reader.A) #для турбин
...
#попробуем так: найдем левую верхнюю точку на кривых HighCurve и LowCurve и относительно нее развернем систему координат так, чтобы кривая лежала примерно горизонтально, после этого ее аппроксимируем и тогда на основе двух этих аппроксимированных кривых построим промежуточные кривые бетта
# lin_coefs=np.polyfit(x=[LowCurveX[0],LowCurveX[-1]],y=[LowCurveY[0],LowCurveY[-1]],deg=1) #находим коэффициенты линейного уравнения k*x+b
# angle_of_rotating=np.deg2rad(75) #вручную задаем угол, на который повернем систему координат так, чтобы все точки кривых LowCurve и HighCurve шли в порядке возрастания x
# r=Rotation.from_rotvec(angle_of_rotating * np.array([0, 0, 1])) #получаем соответствующий вектор вращения

# LowCurveXRot=[]
# LowCurveYRot=[]
# for x,y in zip(LowCurveX,LowCurveY):
#     v0=[x,y,0]
#     v1=r.apply(v0)
#     LowCurveXRot.append(v1[0])
#     LowCurveYRot.append(v1[1])

# HighCurveXRot=[]
# HighCurveYRot=[]
# for x,y in zip(HighCurveX,HighCurveY):
#     v0=[x,y,0]
#     v1=r.apply(v0)
#     HighCurveXRot.append(v1[0])
#     HighCurveYRot.append(v1[1])    

# points_for_scatter=[{'x':LowCurveXRot,'y':LowCurveYRot,'label':'нижние крайние точки rot','s':10,'marker':'x','c':'green'},
#                     {'x':HighCurveXRot,'y':HighCurveYRot,'label':'верхние крайние точки rot','s':10,'marker':'x','c':'orange'},
...
# chart.Chart(points_for_scatter=points_for_scatter,figure_size=(14,18),title='Первичная обработка исходных данных')
...
# LowCurveX.append(2.1),LowCurveY.append(65e-6) #!!!!!эту точку добавляем вручную
# LowCurveX.insert(0,2),LowCurveY.insert(0,1.2) #!!!!!эту точку добавляем вручную
...
x_high=np.linspace(min(HighCurveX)*0.9,max(HighCurveX)*1.1,100)
x_low=np.linspace(min(LowCurveX)*0.9,max(LowCurveX)*1.1,100)
y_high=HighCurve(x_high)
y_low=LowCurve(x_low)
# x_high=HighCurve(y_high)
# x_low=LowCurve(y_low)
...
def BettaCurve(betta):
    ...
    # print('xcrvlow='+str([Xlowmincrv,Xlowmaxcrv]))
    # print('ycrvlow='+str([Ylowmincrv,Ylowmaxcrv]))
    # print('xcrvhigh='+str([Xhighmincrv,Xhighmaxcrv]))
    # print('ycrvhigh='+str([Xhighmincrv,Yhighmaxcrv]))
    # plt.scatter(_x,_y)
    # _x,_y=zip(*sorted(zip(_x,_y)))
    # return UnivariateSplineModified(_x,_y,k=2,s=0.0)
    return np.poly1d(np.polyfit(_y,_x,2))
...
for _i_color in zip(np.linspace(-0.5, 1.5, 9),colors):
    i=_i_color[0]
    color=_i_color[1]
    x_betta=BettaCurve(i)(y)
    _fig.figure.axes[0].plot(x_betta,y,'--r',label='Betta='+str(i),color=color)
...
def f_factory(func,val):#вспомогательная фабрика функций (https://stackoverflow.com/questions/3431676/creating-functions-in-a-loop)
    def f(x):           #сначала я пытался создавать функции внутри цикла ниже, но так не работает, поскольку переменные внутри функции читаются на момент обращения к этой функции, а не на момент ее объявления
        return func(x)*val  # i is now a *local* variable of f_factory and can't ever change
    return f   

#проходимся по всем заданным веткам оборотов, находим угловые коэффициенты k линейного уравнения kx+b на каждой ветке оборотов, на верхнем и нижнем краях этой ветки оборотов на основе 3-х крайних точек (т.е. по трем точкам  создаем линейную регрессию и из регрессии находим угловой коэффициент)
...
for ni in rotations:
    ...
    max_G=G_temp.max()
    max_PR=PR_temp.max()
    G_temp=G_temp/max_G #с помощью деления всего массива на его же максимальное значение мы приводим значения к относительному виду, как показывает практика это влияет на вид аппроксимирующей функции - без нормализации функция может выглядеть криво, особенно если порядки величин X и Y сильно отличаются
    ...
    if type_of_map=="turbine":
        for XY in temp_data.loc[:,['PRrel','Gnrel']].values:
            X_PR=XY[0];Y_Gn=XY[1]
            _func=lambda betta: BettaCurve(betta)(X_PR)-Y_Gn
            _betta=root(_func,x0=0.5)['x'][0]
            print(f'betta={_betta} PR={X_PR} Gn={Y_Gn}')
            # xxx=np.linspace(-3,3,100)
            # yyy=[_func(x) for x in xxx]
            # plt.plot(xxx,yyy)
            Betta_temp.append(_betta)
    elif type_of_map == "compressor":
        for XY in temp_data.loc[:,['PR','G']].values:
            # X_PR=XY[0];Y_G=XY[1]
            X_G=XY[1];Y_PR=XY[0]
            # _func=lambda betta: BettaCurve(betta)(X_R)-Y_G
            _func=lambda betta: BettaCurve(betta)(X_G)-Y_PR
            _betta=root(_func,x0=0.5)['x'][0]
            # print(ni,X_G,Y_PR,_betta)
            Betta_temp.append(_betta)
    ...
    _slope_G_low=np.polyfit(x=low_betta,y=low_G,deg=1)[0]
    ...
    temp_data=temp_data.assign(G=G_temp, PR=PR_temp)
...
_rotations=np.concatenate([rotations[0]*0.85,rotations[0]*0.9,rotations[0]*0.95,rotations,rotations[-1]*1.05,rotations[-1]*1.1,rotations[-1]*1.15],axis=None)

# print('!!!!!!!!!!!!!!!!!!!')
# print(len(_rotations))
# print(slopes_G_low)
# print(len(np.concatenate([slopes_G_low[0],slopes_G_low[0],slopes_G_low[0],slopes_G_low,slopes_G_low[-1],slopes_G_low[-1],slopes_G_low[-1]],axis=None)))


slopes_G_low_func=np.poly1d(np.polyfit(x=_rotations,y=np.concatenate([slopes_G_low[0],slopes_G_low[0],slopes_G_low[0],slopes_G_low,slopes_G_low[-1],slopes_G_low[-1],slopes_G_low[-1]],axis=None),deg=3))
...
slopes_G_high_func=np.poly1d(np.polyfit(x=_rotations,y=np.concatenate([slopes_G_high[0],slopes_G_high[0],slopes_G_high[0],slopes_G_high,slopes_G_high[-1],slopes_G_high[-1],slopes_G_high[-1]],axis=None),deg=3))
...
if type_of_map=="turbine":
    slopes_Gn_low_func=np.poly1d(np.polyfit(x=rotations,y=slopes_Gn_low,deg=2))
    slopes_Gn_high_func=np.poly1d(np.polyfit(x=rotations,y=slopes_Gn_high,deg=2))
    if "L" in temp_data:
        slopes_L_low_func=np.poly1d(np.polyfit(x=rotations,y=slopes_L_low,deg=2))
        slopes_L_high_func=np.poly1d(np.polyfit(x=rotations,y=slopes_L_high,deg=2))
    if "A" in temp_data:
        slopes_A_low_func=np.poly1d(np.polyfit(x=rotations,y=slopes_A_low,deg=2))
        slopes_A_high_func=np.poly1d(np.polyfit(x=rotations,y=slopes_A_high,deg=2))
...
if type_of_map=="compressor":
    ...
    _fig.add_chart(points_for_scatter=points_for_scatter,points_for_plot=points_for_plot,title='Коэффициенты k (slope) линейного уравнения y=k*x+b для разных оборотов и соответствующая регрессия',xlabel='n',ylabel='slope')
...
for ni in rotations:
    ...
    if type_of_map=="turbine":
        ...
        high_Gn_extrapolate=pd.Series([high_extr_func_Gn(val) for val in low_betta_extrapolate])
        if "L" in temp_data:
            low_L_extrapolate=pd.Series([low_extr_func_L(val) for val in low_betta_extrapolate])
            high_L_extrapolate=pd.Series([high_extr_func_L(val) for val in low_betta_extrapolate])            
        if "A" in temp_data:
            low_A_extrapolate=pd.Series([low_extr_func_A(val) for val in low_betta_extrapolate])
            high_A_extrapolate=pd.Series([high_extr_func_A(val) for val in low_betta_extrapolate])   
    ...
    if type_of_map=="compressor":    
        _temp=pd.concat([_G,_PR,_Eff,_betta],axis=1,keys=['G','PR','Eff','betta'])
    elif type_of_map=="turbine":
        _temp=pd.concat([_G,_Gn,_PR,_Eff,_betta],axis=1,keys=['G','Gn','PR','Eff','betta'])
        if "L" in temp_data:
            _temp=pd.concat([_temp,_L],axis=1)
        if "A" in temp_data:
            _temp=pd.concat([_temp,_A],axis=1)
    _temp['n']=ni
...
# if type_of_map=="turbine":
#     quantity_of_graphics=8
# elif type_of_map=='compressor':
#     quantity_of_graphics=6
# fig, axes = plt.subplots(quantity_of_graphics,1)
# fig.set_size_inches(17, quantity_of_graphics*8)
# legend_info=[]
# col=0
G_output=[]
Eff_output=[]
...
for ind,ni_color in enumerate(zip(rotations,colors)):#проходимся по всем заданным веткам оборотов, строим графики для проверки
    ...
    G_func_rel=UnivariateSplineModified(Betta_temp,G_temp,k=5,s=0.0000001)#!!! Здесь нужно правильно выбрать аппроксимирующую функцию, точная сплайн-интерполяция хороша только если исходные данные обладают высокой точностью, иначе лучше использовать аппроксимацию
    G_func_rel_der2=G_func_rel.derivative(2)
    G_func=f_factory(G_func_rel,maxs_G.pop(0))

    Eff_func=UnivariateSplineModified(Betta_temp,Eff_temp,k=4,s=0.0001)#!!! Здесь нужно правильно выбрать аппроксимирующую функцию, точная сплайн-интерполяция хороша только если исходные данные обладают высокой точностью, иначе лучше использовать аппроксимацию
    Eff_func_der2=Eff_func.derivative(2)
    
    PR_func_rel=UnivariateSplineModified(Betta_temp,PR_temp,k=5,s=0.00001)#!!! Здесь нужно правильно выбрать аппроксимирующую функцию, точная сплайн-интерполяция хороша только если исходные данные обладают высокой точностью, иначе лучше использовать аппроксимацию
    PR_func_rel_der2=PR_func_rel.derivative(2)
    PR_func=f_factory(PR_func_rel,maxs_PR.pop(0))
...
#        A_func=Akima1DInterpolatorModified(Betta_temp,A_temp)
        if 'L_temp' in globals():
            L_func_rel=UnivariateSplineModified(Betta_temp,L_temp.tolist(),k=5,s=0.00001)
            def L_func(x):
                return L_func_rel(x)*max_L
        if 'A_temp' in globals():
            A_func_rel=UnivariateSplineModified(Betta_temp,A_temp.tolist(),k=5,s=0.00001)
            def A_func(x):
...
#4.2) проверяем поветочно. Т.е. строим графики исходных данных (обозначены точками) и обработанных кривых отдельно для каждой ветки оборотов
    ...
    for betta_i in bbb:
        ...
        Efff.append(float(Eff_func(betta_i)))
        Efff_der2.append(float(Eff_func_der2(betta_i)))
        if type_of_map=="turbine":
            if 'L_func_rel' in globals():
    ...
    points_for_scatter=[{'x':Betta_temp,'y':G_temp,'label':'G','c':'green'},
                        {'x':Betta_temp,'y':PR_temp,'label':'PR','c':'red'},
                        {'x':Betta_temp,'y':Eff_temp,'label':'Eff','c':'blue'}]
    
    points_for_plot=[{'x':bbb,'y':GGG,'label':'G','c':'green'},
    ...
    _fig2=chart.Chart(points_for_scatter=points_for_scatter,points_for_plot=points_for_plot,title='{} {}'.format('G/PR/Eff = f(betta). Ветка n=',ni),xlabel='betta',dpi=150,figure_size=(14,21))

    # axes3[0].plot(bbb,GGG,label='G',color='green')
    # axes3[0].twinx().plot(bbb,GGG_der2,label='G_der2',color='green',ls=':')
    # axes3[0].scatter(Betta_temp,G_temp,color='green')
    # #    axes3[ind].scatter(Betta_temp2,G_temp2)
    # axes3[0].plot(bbb,PRRR,label='PR',color='red')
    # axes3[0].twinx().plot(bbb,PRRR_der2,label='PR_der2',color='red',ls=':')
    
    # axes3[0].scatter(Betta_temp,PR_temp,color='red')
    if type_of_map=="turbine":
        pass
        # if len(LLL)>0:
        #     axes3[0].plot(bbb,LLL,label='Lambda')
        #     axes3[0].scatter(Betta_temp,L_temp) 
        # if len(AAA)>0:
        #     axes3[0].plot(bbb,AAA,label='Alfa')
        #     axes3[0].scatter(Betta_temp,A_temp)
    
    
    if type_of_map=="turbine":
        pass
        # axes3[1].plot(PRRR,GGG)
        # axes3[1].scatter(PR_temp,G_temp)
        # axes3[1].set_title('PR=f(G). {} {}'.format('ветка n=',ni))
        # axes3[1].set_xlabel('PR')
        # axes3[1].set_ylabel('G')
        # axes3[2].scatter(PR_temp,Eff_temp)
        # axes3[2].plot(PRRR,Efff,color='green')
        # axes3[2].set_title('Eff=f(PR). {} {}'.format('ветка n=',ni))
        # axes3[2].set_xlabel('PR')
    if type_of_map=="compressor":
        _fig2.add_chart(points_for_scatter=[{'y':PR_temp,'x':G_temp}],points_for_plot=[{'y':PRRR,'x':GGG}],title='PR=f(G). {} {}'.format('ветка n=',ni),xlabel='G',ylabel='PR')
        _fig2.add_chart(points_for_scatter=[{'y':Eff_temp,'x':G_temp}],points_for_plot=[{'y':Efff,'x':GGG}],title='Eff=f(G). {} {}'.format('ветка n=',ni),xlabel='G',ylabel='Eff')
        ...
        # axes3[1].plot(GGG,PRRR)
        # axes3[1].scatter(G_temp,PR_temp)
        # axes3[1].set_title('G=f(PR). {} {}'.format('ветка n=',ni))
        # axes3[1].set_xlabel('G')
        # axes3[1].set_ylabel('PR')4
        # axes3[2].scatter(G_temp,Eff_temp)
        # axes3[2].plot(GGG,Efff,color='green')
        # axes3[2].set_title('Eff=f(G). {} {}'.format('ветка n=',ni))
        # axes3[2].set_xlabel('G')
        # axes3[3].scatter(PR_temp,Eff_temp)
        # axes3[3].plot(PRRR,Efff,color='green')
        # axes3[3].set_title('Eff=f(PR). {} {}'.format('ветка n=',ni))
        # axes3[3].set_xlabel('PR')
        # axes3[3].set_ylabel('Eff')
        # axes3[3].set_ylim([0.2,max_Eff*1.1]) #ЗДЕСЬ
        # axes3[3].grid()
#    axes3[ind].scatter(Betta_temp2,PR_temp2)
#     axes3[0].plot(bbb,Efff,label='Eff')
#     axes3[0].scatter(Betta_temp,Eff_temp)
# #    axes3[ind].scatter(Betta_temp2,Eff_temp2)
#     axes3[0].set_title('{} {}'.format('ветка n=',ni))
    
# #    axes3[2].set_title('Eff=f(G). {} {}'.format('ветка n=',ni))
#     axes3[0].legend()
# #    axes3[0].twinx().legend()
#     axes3[0].set_xlabel('betta')
#     axes3[0].grid()

    # axes3[1].grid()
    
    # axes3[2].set_ylabel('Eff')
    # axes3[2].set_ylim([0.2,max_Eff*1.1]) #ЗДЕСЬ
    # axes3[2].grid()
#    axes3[1].plot(bbb,PRRR)
#    axes3[2].plot(bbb,Efff)

chart.Chart(points_for_scatter=scatter_GPR,points_for_plot=plot_GPR,title='PR=f(G) для компрессора или PR=f(G*n) для турбины.',xlabel='G или G*n',ylabel='PR',dpi=150,figure_size=(14,7))
chart.Chart(points_for_scatter=scatter_GEff,points_for_plot=plot_GEff,title='Eff=f(G) для компрессора или Eff=f(G*n) для турбины.',xlabel='G или G*n',ylabel='Eff',dpi=150,figure_size=(14,7))
...
def Parameters_out(betta,n): 
    ...
    s_for_G=interp1d([-1,0,1,2],[0.001,0.00001,0.00001,0.001],fill_value=(1,1))
    s_for_PR=interp1d([-1,0,1,2],[0.001,0.00001,0.00001,0.001],fill_value=(1,1))
    s_for_Eff=interp1d([-1,0,1,2],[0.01,0.0001,0.0001,0.01],fill_value=(1,1))
    max_G=max(G_vector)
    max_PR=max(PR_vector)
    G_rel=UnivariateSplineModified(rotations,G_vector/max_G,k=5,s=s_for_G(betta))
    G=f_factory(G_rel,max_G)
    G_der2=G_rel.derivative(2)
    PR_rel=UnivariateSplineModified(rotations,PR_vector/max_PR,k=5,s=s_for_PR(betta))
    PR=f_factory(PR_rel,max_PR)
    PR_der2=PR_rel.derivative(2)
    Eff=UnivariateSplineModified(rotations,Eff_vector,k=5,s=s_for_Eff(betta))
    Eff_der2=Eff.derivative(2)
    if type_of_map=="turbine":
#        L=Akima1DInterpolatorModified(rotations,L_vector)
#        A=Akima1DInterpolatorModified(rotations,A_vector)
        if len(L_vector)>0:
            max_L=max(L_vector)
            L_rel=UnivariateSplineModified(rotations,L_vector,k=5,s=0.0001)
            L=f_factory(L_rel,max_L)
        if len(A_vector)>0:
            max_A=max(A_vector)
            A_rel=UnivariateSplineModified(rotations,A_vector,k=5,s=0.0001)
...
for ind,betta_i_colors in enumerate(zip(bbb,colors)):
    ...
    if type_of_map=="turbine":
        ...
        L_v2=[]  
    for n_i in rot_v2:
        G_v2.append(G_f(n_i).tolist())
        G_v2_der2.append(G_f_der2(n_i))
        PR_v2.append(PR_f(n_i).tolist())
        PR_v2_der2.append(PR_f_der2(n_i))
        Eff_v2.append(Eff_f(n_i).tolist())
    ...
    if type_of_map=="turbine":
        pass
        # if len(A_v2)>0:
        #     points_for_plot=[{'x':rot_v2,'y':A_v2,'label':betta_i,'c':color}]
        #     points_for_scatter=[{'x':rot_v,'y':A_v,'c':color}]
        #     _fig3.add_chart(points_for_scatter=points_for_scatter,points_for_plot=points_for_plot,title='{}'.format('Alfa=f(n) при разных betta'),ylabel='Alfa',xlabel='n')
        # if len(L_v2)>0:
        #     points_for_plot=[{'x':rot_v2,'y':L_v2,'label':betta_i,'c':color}]
        #     points_for_scatter=[{'x':rot_v,'y':L_v,'c':color}]
        #     _fig3.add_chart(points_for_scatter=points_for_scatter,points_for_plot=points_for_plot,title='{}'.format('Lambda=f(n) при разных betta'),ylabel='Lambda',xlabel='n')
    _fig3=chart.Chart(points_for_scatter=scatter_G,points_for_plot={'x':rot_v2,'y':G_v2,'label':'G','c':'blue'},title='{} {} {}'.format('G=f(n).','Ветка betta=',betta_i),ylabel='G',xlabel='n',dpi=150,figure_size=(17,15))        


            
_fig4=chart.Chart(points_for_scatter=scatter_G,points_for_plot=plot_G,title='{}'.format('G=f(n) при разных betta'),ylabel='G',xlabel='n',dpi=150,figure_size=(14,21))
_fig4.add_chart(points_for_scatter=scatter_PR,points_for_plot=plot_PR,title='{}'.format('PR=f(n) при разных betta'),ylabel='PR',xlabel='n')        
_fig4.add_chart(points_for_scatter=scatter_Eff,points_for_plot=plot_Eff,title='{}'.format('Eff=f(n) при разных betta'),ylabel='Eff',xlabel='n')
    # fig, axes = plt.subplots(3,1)
    # fig.set_size_inches(17, 15)
    
    # axes[0].plot(rot_v2,G_v2,label='G',color='blue')
    # axes[0].scatter(rot_v,G_v,label='G_initial',color='blue')
    # axes_twin=axes[0].twinx()
    # axes_twin.plot(rot_v2,G_v2_der2,label='G_der2',color='blue',ls=':')
    # axes[0].set_title('{} {} {}'.format('G=f(n).','Ветка betta=',betta_i))
    # axes[0].set_xlabel('n')
    # axes[0].set_ylabel('G')
    # axes_twin.set_ylabel('G_der2')
    # axes[0].legend()
    # axes_twin.legend()

    # axes[1].plot(rot_v2,PR_v2,label='PR',color='red')
    # axes[1].scatter(rot_v,PR_v,label='PR_initial',color='red')
    # axes_twin2=axes[1].twinx()
    # axes_twin2.plot(rot_v2,PR_v2_der2,label='PR_der2',color='red',ls=':')
    # axes[1].set_title('{} {} {}'.format('PR=f(n).','Ветка betta=',betta_i))
    # axes[1].set_xlabel('n')
    # axes[1].set_ylabel('PR')
    # axes_twin2.set_ylabel('PR_der2')
    # axes[1].legend()
    # axes_twin2.legend()
    
    # axes[2].plot(rot_v2,Eff_v2,label='Eff',color='green')
    # axes[2].scatter(rot_v,Eff_v,label='Eff_initial',color='green')
    # axes_twin3=axes[2].twinx()
    # axes_twin3.plot(rot_v2,Eff_v2_der2,label='Eff_der2',color='green',ls=':')
    # axes[2].set_title('{} {} {}'.format('Eff=f(n).','Ветка betta=',betta_i))
    # axes[2].set_xlabel('n')
    # axes[2].set_ylabel('Eff')
    # axes_twin3.set_ylabel('Eff_der2')
    # axes[2].legend()
    # axes_twin3.legend()
#    axes[0].scatter(Betta_temp,G_temp,color='green')
        
#    axes4[2*ind].plot(rot_v2,PR_v2,label='PR')
#    axes4[2*ind].scatter(rot_v,PR_v)
#    axes4[2*ind].plot(rot_v2,Eff_v2,label='Eff')
#    axes4[2*ind].scatter(rot_v,Eff_v)
"""
...
size_n=100
size_betta=100
n_min=rotations[0]-0.05
n_max=rotations[-1]+0.05
betta_min=-0.4
betta_max=1.4
...
for ind_n, n_i in enumerate(n_mainrezult):
    for ind_betta, betta_i in enumerate(betta_mainrezult):
        ...
        Eff_mainrezult[ind_n,ind_betta]=rez['Eff']
        if type_of_map=="turbine":
            if 'A' in rez:
...
Eff_Func4=RectBivariateSpline(n_mainrezult,betta_mainrezult, Eff_mainrezult, bbox=[n_min, n_max, betta_min, betta_max], kx=5, ky=5, s=0.0001)
G_Func4=RectBivariateSpline(n_mainrezult,betta_mainrezult, G_mainrezult, bbox=[n_min, n_max, betta_min, betta_max], kx=5, ky=5, s=0.0001)
PR_Func4=RectBivariateSpline(n_mainrezult,betta_mainrezult, PR_mainrezult,bbox=[n_min, n_max, betta_min, betta_max], kx=5, ky=5, s=0.0001)
#A_Func4=RectBivariateSpline(n_v, betta_v, A_m, bbox=[0, 1.3, -0.5, 1.5], kx=3, ky=3, s=0)
#L_Func4=RectBivariateSpline(n_v, betta_v, L_m, bbox=[0, 1.3, -0.5, 1.5], kx=3, ky=3, s=0)
...
name_of_file=date_text+' '+name_of_file[:-4]+'_output.dat'
...
ni = np.linspace(n_min, n_max, size_n)
ni2 = np.arange(n_min, n_max, 0.025)
bi = np.linspace(betta_min, betta_max, size_betta)
bi2 = np.arange(betta_min, betta_max+0.00001, 0.2)
...
lev = [0.3,0.4,0.5,0.6,0.65,0.7,0.75,0.8,0.81,0.82,0.83,0.84,0.85,0.86,0.87,0.88,0.89,0.9] #!!!!!!! при необходимости подправить значения изолиний кпд
...
if type_of_map=='compressor':
    ...
    axes3.scatter(G,PR,color='black',s=5)
    axes3.plot(Gv2,PRv2,color='black',linewidth=1.2,linestyle='dotted',label=(0.3,0.4,0.5,0.6,0.7,0.8,0.9,1.0))
    axes3.plot(Gv3,PRv3,color='black',linewidth=1.2,linestyle='dotted',label=(0,0.2,0.4,0.6,0.8,1))
    ...
    axes3.plot(PRv2,Gnv2,color='white',linewidth=1.2,linestyle='dotted',label=(0.3,0.4,0.5,0.6,0.7,0.8,0.9,1.0))
    axes3.plot(PRv3,Gnv3,color='white',linewidth=1.2,linestyle='dotted',label=(0,0.2,0.4,0.6,0.8,1))
    axes3.set_ylabel('G*n, кг/с',fontsize=20)
    axes3.set_xlabel('PR',fontsize=20)
    for i,nx in enumerate(ni2):
        axes3.text(PRv2[0,i]-0.05,Gnv2[0,i]-0.02,np.round(nx,3),fontsize=14)
    for i,bettax in enumerate(bi2):
        axes3.text(PRv3[-1,i],Gnv3[-1,i]+1,np.round(bettax,2),fontsize=14)
...
#7.2)отдельно сравним исходные точки кпд с получившимися
fig5, axes5 = plt.subplots(3,1)
fig5.set_size_inches(17, 30)
axes5[0].set_ylim([0.6,max_Eff*1.1])
axes5[0].grid()
axes5[0].set_xlabel('G, кг/с',fontsize=20)
axes5[0].set_ylabel('Eff',fontsize=20)
axes5[0].set_title('Eff=f(G): сверяем исходные данные (точки) с итоговой аппроксимирующей функцией')

axes5[1].set_ylim([0.6,max_Eff*1.1])
axes5[1].grid()
axes5[1].set_xlabel('PR',fontsize=20)
axes5[1].set_ylabel('Eff',fontsize=20)
axes5[1].set_title('Eff=f(PR): сверяем исходные данные (точки) с итоговой аппроксимирующей функцией')

axes5[2].grid()
if type_of_map=='compressor':
    axes5[2].set_xlabel('G, кг/с',fontsize=20)
    axes5[2].set_ylabel('PR',fontsize=20)
    axes5[2].set_title('PR=f(G): сверяем исходные данные (точки) с итоговой аппроксимирующей функцией')
if type_of_map=='turbine':
    axes5[2].set_xlabel('PR',fontsize=20)
    axes5[2].set_ylabel('G*n',fontsize=20)
    axes5[2].set_title('G*n=f(PR): сверяем исходные данные (точки) с итоговой аппроксимирующей функцией')
...
for ni_color in zip(rotations,colors):
    ...
    PRvect=[]
    Effvect=[]
    G_points=reader.G[reader.n==ni]
    if type_of_map=='turbine':
        Gn_points=reader.Gn[reader.n==ni]
    PR_points=reader.PR[reader.n==ni]
    Eff_points=reader.Eff[reader.n==ni]
    for betta in np.linspace(0.0, 1.0, 50):
        ...
        if type_of_map=='compressor':
            axes5[2].plot(Gvect,PRvect,color=color,linewidth=0.5)
            axes5[2].scatter(G_points,PR_points,color=color)
            axes5[0].plot(G_points,Eff_points,color=color,marker='.',ls='--',linewidth=0.2)
        if type_of_map=='turbine':
            axes5[2].plot(PRvect,Gvect,color=color,linewidth=0.5)
            axes5[2].scatter(PR_points,Gn_points,color=color)
            axes5[0].plot(Gn_points,Eff_points,color=color,marker='.',ls='--',linewidth=0.2)
        axes5[0].plot(Gvect,Effvect,color=color,linewidth=0.5)
        axes5[1].plot(PRvect,Effvect,color=color,linewidth=0.5)
#        axes5[2].plot(Gvect,PRvect,color=color,linewidth=0.5)
        
        axes5[1].plot(PR_points,Eff_points,color=color,marker='.',ls='--',linewidth=0.2)
#        axes5[2].scatter(G_points,PR_points,color=color)
