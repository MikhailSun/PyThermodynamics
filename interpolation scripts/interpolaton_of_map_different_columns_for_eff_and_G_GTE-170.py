...
name_of_file="2020 09 аэродинамические характеристики Т ГТЭ-170 по результатам CFD\GTE170_turbine_CFD_map_Zhivirikhin_Mikhail_2020_09.csv"#!!!указать файл
...
if type_of_map=="turbine":
    ...
    # points_for_scatter2=[{'x':reader.PR,'y':reader.Gn,'label':'исходные данные','s':1}]
    # _fig2=chart.Chart(points_for_scatter=points_for_scatter2,figure_size=(14,18),title='Первичная обработка исходных данных')
...
for ind,ni_color in enumerate(zip(rotations,colors)):#проходимся по всем заданным веткам оборотов, строим графики для проверки
    ...
    Eff_func_rel=UnivariateSplineModified(Betta_temp,Eff_temp,k=3,s=0.0001)#!!! Здесь нужно правильно выбрать аппроксимирующую функцию, точная сплайн-интерполяция хороша только если исходные данные обладают высокой точностью, иначе лучше использовать аппроксимацию
    Eff_func_rel_der2=Eff_func_rel.derivative(2)
    Eff_func=f_factory(Eff_func_rel,maxs_Eff.pop(0))

    
    PR_func_rel=UnivariateSplineModified(Betta_temp,PR_temp,k=4,s=0.0001)#!!! Здесь нужно правильно выбрать аппроксимирующую функцию, точная сплайн-интерполяция хороша только если исходные данные обладают высокой точностью, иначе лучше использовать аппроксимацию
...
#        A_func=Akima1DInterpolatorModified(Betta_temp,A_temp)
        if 'Gn_temp' in globals():
            Gn_func_rel=UnivariateSplineModified(Betta_temp,Gn_temp.tolist(),k=5,s=0.0001)
            Gn_func_rel_der2=Gn_func_rel.derivative(2)
            Gn_func=f_factory(Gn_func_rel,maxs_Gn.pop(0))
        if 'L_temp' in globals():
            L_func_rel=UnivariateSplineModified(Betta_temp,L_temp.tolist(),k=5,s=0.0005)
            L_func_rel_der2=L_func_rel.derivative(2)
            L_func=f_factory(L_func_rel,maxs_L.pop(0))
            # def L_func(x):
            #     return L_func_rel(x)*max_L
        if 'A_temp' in globals():
            A_func_rel=UnivariateSplineModified(Betta_temp,A_temp.tolist(),k=5,s=0.0001)
            A_func_rel_der2=A_func_rel.derivative(2)
            A_func=f_factory(A_func_rel,maxs_A.pop(0))
            # def A_func(x):
...
def Parameters_out(betta,n): 
    ...
    s_for_G=interp1d([-1,0,1,2],[0.001,0.0001,0.0001,0.001],fill_value=(1,1))
    s_for_PR=interp1d([-1,0,1,2],[0.001,0.0001,0.0001,0.001],fill_value=(1,1))
    s_for_Eff=interp1d([-1,0,1,2],[0.01,0.001,0.001,0.01],fill_value=(1,1))
    max_G=max(G_vector)
    ...
    G_rel=UnivariateSplineModified(rotations,[x/max_G for x in G_vector],k=5,s=s_for_G(betta))
    G=f_factory(G_rel,max_G)
    G_der2=G_rel.derivative(2)
    PR_rel=UnivariateSplineModified(rotations,[x/max_PR for x in PR_vector],k=5,s=s_for_PR(betta))
    PR=f_factory(PR_rel,max_PR)
    PR_der2=PR_rel.derivative(2)
    Eff_rel=UnivariateSplineModified(rotations,[x/max_Eff for x in Eff_vector],k=4,s=s_for_Eff(betta))
...
if type_of_map=="compressor":
    _fig4=chart.Chart(points_for_scatter=scatter_G,points_for_plot=[{'x':rot_v2,'y':G_v2,'label':'G','c':'blue'}],title='{} {} {}'.format('G=f(n).','Ветка betta=',betta_i),ylabel='G',xlabel='n',dpi=150,figure_size=(17,15))        
...
ni2 = np.arange(n_min, n_max, 0.05)
...
lev = [0.2,0.4,0.5,0.6,0.7,0.75,0.8,0.81,0.82,0.83,0.84,0.85,0.86,0.87,0.88,0.89,0.9,0.91,0.92,0.93,0.94,0.95] #!!!!!!! при необходимости подправить значения изолиний кпд
...
if type_of_map=='compressor':
    ...
    for i,nx in enumerate(ni2):
        axes3.text(PRv2[0,i]-0.1,Gnv2[0,i]-0.00000002,np.round(nx,3),fontsize=14)
    for i,bettax in enumerate(bi2):
        axes3.text(PRv3[-1,i],Gnv3[-1,i]+0.00000001,np.round(bettax,2),fontsize=14)
