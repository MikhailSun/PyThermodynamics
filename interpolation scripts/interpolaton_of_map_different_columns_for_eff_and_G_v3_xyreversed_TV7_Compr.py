...
name_of_file="2020 08 аэродинамические характеристики ОЦК ТВ7-117 по результатаам CFD\ACC CFD 2020 09.csv"#!!!указать файл
...
for ind,ni_color in enumerate(zip(rotations,colors)):#проходимся по всем заданным веткам оборотов, строим графики для проверки
    ...
    G_func_rel=UnivariateSplineModified(Betta_temp,G_temp,k=3,s=0.00001)#!!! Здесь нужно правильно выбрать аппроксимирующую функцию, точная сплайн-интерполяция хороша только если исходные данные обладают высокой точностью, иначе лучше использовать аппроксимацию
    ...
    PR_func_rel=UnivariateSplineModified(Betta_temp,PR_temp,k=3,s=0.00001)#!!! Здесь нужно правильно выбрать аппроксимирующую функцию, точная сплайн-интерполяция хороша только если исходные данные обладают высокой точностью, иначе лучше использовать аппроксимацию
...
def Parameters_out(betta,n): 
    ...
    s_for_G=interp1d([-1,0,1,2],[0.001,0.00005,0.00005,0.001],fill_value=(1,1))
    s_for_PR=interp1d([-1,0,1,2],[0.001,0.00005,0.00005,0.001],fill_value=(1,1))
    ...
    G_rel=UnivariateSplineModified(rotations,[x/max_G for x in G_vector],k=3,s=s_for_G(betta))
    G=f_factory(G_rel,max_G)
    G_der2=G_rel.derivative(2)
    PR_rel=UnivariateSplineModified(rotations,[x/max_PR for x in PR_vector],k=3,s=s_for_PR(betta))
    PR=f_factory(PR_rel,max_PR)
    PR_der2=PR_rel.derivative(2)
    Eff_rel=UnivariateSplineModified(rotations,[x/max_Eff for x in Eff_vector],k=3,s=s_for_Eff(betta))
...
Eff_Func4=RectBivariateSpline(n_mainrezult,betta_mainrezult, Eff_mainrezult, bbox=[n_min, n_max, betta_min, betta_max], kx=4, ky=4, s=0.0001)
G_Func4=RectBivariateSpline(n_mainrezult,betta_mainrezult, G_mainrezult, bbox=[n_min, n_max, betta_min, betta_max], kx=5, ky=5, s=0.00005)
PR_Func4=RectBivariateSpline(n_mainrezult,betta_mainrezult, PR_mainrezult,bbox=[n_min, n_max, betta_min, betta_max], kx=5, ky=5, s=0.00005)
if type_of_map=='turbine':
...
lev = [0.2,0.4,0.5,0.55,0.6,0.65,0.7,0.71,0.72,0.73,0.74,0.75,0.76,0.77,0.78,0.79,0.8] #!!!!!!! при необходимости подправить значения изолиний кпд
...
if type_of_map=='compressor':
    ...
    axes3.plot(Gv2,PRv2,color='black',linewidth=1.2,linestyle='dotted',label=(0.75,0.775,0.8,0.825,0.85,0.875,0.9,0.925,0.95,0.975,1.0,1.025,1.05))
