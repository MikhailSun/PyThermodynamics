...
name_of_file="2020 11 аэродинамические характеристики компрессора ГТЭ-170 по результатам CFD\AC CFD 2020 11 A-30.csv"#!!!указать файл
...
LowCurveY[0]=LowCurveY[0]*0.95;LowCurveY[1]=LowCurveY[1]*0.95
...
x=np.linspace(Xmin*0.95,Xmax*1.05,100)
y=np.linspace(Ymin*0.95,Ymax*1.05,100)
x_high=np.linspace(min(HighCurveX)*0.95,max(HighCurveX)*1.05,100)
x_low=np.linspace(min(LowCurveX)*0.95,max(LowCurveX)*1.05,100)
# x_high=HighCurve(y)
# x_low=LowCurve(y)
...
slopes_Eff_high[-2]=(slopes_Eff_high[-3]+slopes_Eff_high[-1])/2
slopes_Eff_high_func=np.poly1d(np.polyfit(x=_rotations2,y=np.concatenate([slopes_Eff_high,slopes_Eff_high[-1],slopes_Eff_high[-1],slopes_Eff_high[-1]],axis=None),deg=3))
...
for ind,ni_color in enumerate(zip(rotations,colors)):#проходимся по всем заданным веткам оборотов, строим графики для проверки
    ...
    G_func_rel=UnivariateSplineModified(Betta_temp,G_temp,k=3,s=0.0001)#!!! Здесь нужно правильно выбрать аппроксимирующую функцию, точная сплайн-интерполяция хороша только если исходные данные обладают высокой точностью, иначе лучше использовать аппроксимацию
    ...
    PR_func_rel=UnivariateSplineModified(Betta_temp,PR_temp,k=3,s=0.0001)#!!! Здесь нужно правильно выбрать аппроксимирующую функцию, точная сплайн-интерполяция хороша только если исходные данные обладают высокой точностью, иначе лучше использовать аппроксимацию
...
def Parameters_out(betta,n): 
    ...
    s_for_G=interp1d([-1,0,1,2],[0.000001,0.00000001,0.00000001,0.000005],fill_value=(1,1))
    s_for_PR=interp1d([-1,0,1,2],[0.0000001,0.00000001,0.00000001,0.0000001],fill_value=(1,1))
    s_for_Eff=interp1d([-1,0,1,2],[0.0001,0.000001,0.00005,0.0001],fill_value=(1,1))
    max_G=max(G_vector)
...
max_eff=scales['Eff']
lev = [0.5,0.6,0.7,0.725,0.75,0.775,0.8,0.81,0.82,0.825,0.83,0.835,0.84,0.845,0.85] #!!!!!!! при необходимости подправить значения изолиний кпд
