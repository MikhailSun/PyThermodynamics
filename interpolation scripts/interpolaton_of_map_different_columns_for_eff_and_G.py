...
for nx in rotations:
    ...
    if type_of_map=="turbine":
        HighCurveX.append(temp_data.PR.iloc[-1])
        HighCurveY.append(temp_data.Gn.iloc[-1])
        LowCurveX.append(temp_data.PR.iloc[0])
        LowCurveY.append(temp_data.Gn.iloc[0])
    elif type_of_map=="compressor":
        HighCurveY.append(temp_data.PR.iloc[-1])
        HighCurveX.append(temp_data.G.iloc[-1])
        LowCurveY.append(temp_data.PR.iloc[0])
        LowCurveX.append(temp_data.G.iloc[0])
...
# HighCurve=np.poly1d(np.polyfit(HighCurveX,HighCurveY,2))
# LowCurve=np.poly1d(np.polyfit(LowCurveX,LowCurveY,3))
# HighCurve=np.poly1d(np.polyfit([3.458, 3.8880000000000003, 4.398],[0.038286, 0.14369600000000002, 0.24910600000000002],2))
...
#Проверяем все, что насчитали выше на графике
...
if type_of_map=="turbine":
    points_for_scatter=[{'x':reader.PR,'y':reader.Gn,'label':'исходные данные','s':1},
    ...
    points_for_scatter=[{'x':reader.G,'y':reader.PR,'label':'исходные данные','s':1},
                    {'x':HighCurveX,'y':HighCurveY,'label':'верхние крайние точки','s':10,'marker':'x','c':'red'},
                    {'x':LowCurveX,'y':LowCurveY,'label':'нижние крайние точки','s':10,'marker':'x','c':'blue'},
                    {'x':[Xlowmin,Xlowmid,Xlowmax],'y':[Ylowmin,Ylowmid,Ylowmax],'label':'нижние опорные точки','s':20,'marker':'o','c':'black'},
                    {'x':[Xlowmincrv,Xlowmidcrv,Xlowmaxcrv],'y':[Ylowmincrv,Ylowmidcrv,Ylowmaxcrv],'label':'нижние опорные точки на кривой','s':30,'marker':'+','c':'black'},
...
if type_of_map=="turbine":
    plt.ylim(min(reader.Gn)*0.1,max(reader.Gn)*1.2)
...
for ni in rotations:
    ...
    if type_of_map=="turbine":
        for XY in temp_data.loc[:,['PR','Gn']].values:
            X_PR=XY[0];Y_Gn=XY[1]
