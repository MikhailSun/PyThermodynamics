...
slopes_G_high_func=Akima1DInterpolatorModified(x=_rotations,y=np.concatenate([slopes_G_high[0],slopes_G_high[0],slopes_G_high[0],slopes_G_high,slopes_G_high[-1],slopes_G_high[-1],slopes_G_high[-1]],axis=None))
...
slopes_PR_high_func=np.poly1d(np.polyfit(x=rotations,y=slopes_PR_high,deg=3))
...
slopes_Eff_high_func=np.poly1d(np.polyfit(x=rotations,y=slopes_Eff_high,deg=3))
...
for ind,ni_color in enumerate(zip(rotations,colors)):#проходимся по всем заданным веткам оборотов, строим графики для проверки
    ...
    G_func_rel=UnivariateSplineModified(Betta_temp,G_temp,k=3,s=0.00005)#!!! Здесь нужно правильно выбрать аппроксимирующую функцию, точная сплайн-интерполяция хороша только если исходные данные обладают высокой точностью, иначе лучше использовать аппроксимацию
    G_func_rel_der2=G_func_rel.derivative(2)
    G_func=f_factory(G_func_rel,rel_maximums['G'].pop(0),rel_minimums['G'].pop(0))

    Eff_func_rel=UnivariateSplineModified(Betta_temp,Eff_temp,k=3,s=0.00001)#!!! Здесь нужно правильно выбрать аппроксимирующую функцию, точная сплайн-интерполяция хороша только если исходные данные обладают высокой точностью, иначе лучше использовать аппроксимацию
...
def Parameters_out(betta,n): 
    ...
    s_for_G=interp1d([-1,0,1,2],[0.000001,0.0000001,0.0000001,0.000001],fill_value=(1,1))
    s_for_PR=interp1d([-1,0,1,2],[0.000001,0.0000001,0.0000001,0.000001],fill_value=(1,1))
    # s_for_Eff=interp1d([-1,0,0.85,0.9,2],[0.0001,0.000005,0.000005,0.0001,0.0005],fill_value=(1,1))
    s_for_Eff=interp1d([-1,0,1,2],[0.0001,0.000005,0.000005,0.0001],fill_value=(1,1))
    max_G=max(G_vector)
...
max_eff=scales['Eff']
lev = [0.7,0.75,0.8,0.81,0.82,0.83,0.84,0.85,0.86,0.87,0.88,0.885,0.89,0.895,0.9] #!!!!!!! при необходимости подправить значения изолиний кпд
